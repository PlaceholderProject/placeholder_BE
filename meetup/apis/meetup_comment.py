...
@meetup_router.get(
    "{meetup_id}/comment",
    response=MeetupCommentListSchema,
    by_alias=True,
    tags=["MeetupComment"],
)
@handle_exceptions
...
@meetup_comment_router.put("{comment_id}", response=CommentSchema, auth=JWTAuth(), by_alias=True)
@handle_exceptions
...
@meetup_comment_router.delete("{comment_id}", response=None, auth=JWTAuth(), by_alias=True)
@handle_exceptions
